 ...
 * @Last Modified time: 2021-01-17 17:00:00
 *************************************************************/
...
struct values
{
  ...
  uint8_t radio_enable:1; // есть ли радио модуль
} params;
...
void configModeCallback (WiFiManager *myWiFiManager) {
  ...
  Serial.println(F("WiFiManager Waiting"));
  Serial.print(F("IP: "));
...
void setup()
{
  ...
  WiFiManagerParameter custom_token("blynk_token", "blynk token", "", 33);
  ...
  custom_token.setValue(saved_data.token, sizeof(saved_data.token));
  ...
  wm.startConfigPortal("WM_ConnectAP", "password");

  strncpy(saved_data.token, custom_token.getValue(), sizeof(saved_data.token));
  DEBUG_PRINTLN("\nSSID: "+wm.getWiFiSSID()+"\nSSID2: "+wm.getWiFiSSID(true)+"\nPASS: "+wm.getWiFiPass()+"\nPASS2: "+wm.getWiFiPass(true)+"\nTOKEN: "+String(saved_data.token));

  Blynk.begin(saved_data.token, wm.getWiFiSSID().c_str(), wm.getWiFiPass().c_str());
